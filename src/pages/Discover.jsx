...
const Discover = () => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        if (usersList.length === 0) {
          console.log("No users found in DB, using dummy data");
          setUsers(dummyUsers);
        } else {
          setUsers(usersList);
        }
  ...
  const filteredUsers = users.filter(user => {
    ...
    const religionMatch = religionFilter === '' || (user.religion || '').toLowerCase().includes(religionFilter.toLowerCase()); // Changed to check religion field directly
    return nameMatch && ageMatch && locationMatch && educationMatch && professionMatch && religionMatch;
  });

  /* Debug Logs */
  console.log("Discover State:", { isLoaded, usersCount: users.length, filteredCount: filteredUsers.length });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#182384] to-[#ce759a]">
        <div className="text-center">
          ...
          <h2 className="text-2xl text-white font-bold animate-pulse">Summoning potential matches...</h2>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-[#182384] to-[#ce759a] text-white font-sans perspective-1000 overflow-hidden relative">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 text-center relative z-10 transform-gpu pt-24">
        <div className={`mb-8 transform transition-all duration-1000 ease-out ${isLoaded ? 'animate-fade-in opacity-100 translate-y-0 rotate-x-0' : 'opacity-0 translate-y-12 rotate-x-45'
          }`}>
        ...
        <div className={`bg-white/10 backdrop-blur-md rounded-2xl p-8 shadow-2xl border border-white/20 transform transition-all duration-1000 ease-out hover:scale-105 hover:rotate-x-3 hover:shadow-[0_25px_50px_rgba(0,0,0,0.3)] preserve-3d group ${isLoaded ? 'animate-fade-in opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
        ...
        <div className={`mt-8 transform transition-all duration-1000 ease-out ${isLoaded ? 'animate-fade-in opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
      ...
      <div className="max-w-7xl mx-auto grid gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 px-4 pb-24 perspective-2000 relative z-10">
        {filteredUsers.length === 0 && isLoaded ? (
          <div className="col-span-full text-center py-20 animate-fade-in">
            ...
            <h2 className="text-3xl font-bold text-white mb-2">No profiles found yet</h2>
            <p className="text-pink-100 text-lg max-w-md mx-auto">
              It looks like you're one of the first ones here!
              Invite your friends to join or try clearing your filters.
            </p>
            <button
              onClick={clearFilters}
              className="mt-6 bg-white/20 hover:bg-white/30 text-white px-6 py-2 rounded-full backdrop-blur-sm transition-all"
            >
              Clear Filters
            </button>
          </div>
          ...
          filteredUsers.map((user, idx) => (
            ...
            >
              <div className="relative overflow-hidden">
                <img
                  src={user.profilePicUrl || user.image || "https://randomuser.me/api/portraits/lego/1.jpg"}
                  alt={user.name}
              ...
              <div className="p-4 transform transition-all duration-700 group-hover:translate-y-1 relative">
                ...
                <p className="italic text-pink-100 transform transition-all duration-500 group-hover:translate-x-2 group-hover:text-pink-200 mt-1 relative z-10 truncate">
                  "{user.about || user.bio || 'No bio available'}"
                </p>

                {/* Enhanced Tags with Wave Animation - using hobbies array if available */}
                <div className="flex flex-wrap gap-2 mt-3 relative z-10 h-16 overflow-hidden">
                  {(Array.isArray(user.hobbies) ? user.hobbies : (user.tags || [])).slice(0, 3).map((tag, i) => (
                    <span
                      key={i}
                      className={`${tagColors[i % tagColors.length]} text-white text-xs px-3 py-1.5 rounded-full transform transition-all duration-500 hover:scale-125 hover:rotate-6 hover:-translate-y-2 shadow-lg cursor-pointer hover:shadow-xl`}
                      style={{
